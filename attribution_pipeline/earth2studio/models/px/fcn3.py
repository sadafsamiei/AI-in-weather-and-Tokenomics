...
class FCN3(torch.nn.Module, AutoModelMixin, PrognosticMixin):
    ...
    #     print(f"x in forward 1: {x.shape}")
    #     coords = OrderedDict({"batch": np.zeros((1,), dtype=int), **coords})
    #     print("x shape in foward before squeeze", x.shape)
    #     x = x.squeeze(2)
    #     print("x shape in foward after squeeze",x.shape)
    ...
    #                 )
    #     x = x.unsqueeze(2)
    #     print(f"x in forward 2: {x.shape}")
    ...
    def _forward(
        ...
        for j, _ in enumerate(coords["batch"]):
            ...
            for i, t in enumerate(coords["time"]):
                ...
                frames.append(y) 
            batches.append(torch.cat(frames, dim=0))
        x_next = torch.cat(batches, dim=0) 
        # x_next = x_next.unsqueeze(2)
        print("x_next", x_next.shape)
        return x_next  # removed the output_coords return
